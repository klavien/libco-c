...
static int CreateTcpSocket(const unsigned short shPort,const char *pszIP,bool bReuse)
{
...
static void *poll_routine( void *arg )
{
	...
	{
		int fd = CreateTcpSocket(0,"*",false);
		SetNonBlock( fd );
		v->t[i].fd = fd;

		int ret = connect(fd,(struct sockaddr*)(&(v->t[i].addr)),sizeof( v->t[i].addr )); 
		printf("co %p connect i %ld ret %d errno %d (%s)\n",
			co_self(),i,ret,errno,strerror(errno));
	}
	struct pollfd *pf = (struct pollfd*)calloc( 1,sizeof(struct pollfd) * v->cnt );
	...
	printf("co %p task cnt %d fire %ld\n",
			co_self(),v->cnt,fireCnt);
	return 0;
}
int main(int argc,char *argv[])
{
	vec v;
	v.cnt=(argc-1)/2;
	v.t=malloc(v.cnt*sizeof(task_t));
	int idx=0;
	for(int i=1;i<argc;i+=2,idx++)
	{
		printf("idx:%d\n",v.cnt);
		SetAddr( argv[i],atoi(argv[i+1]),&((v.t[idx]).addr));
	}
...
//------------------------------------------------------------------------------------
	...
	for(int i=0;i<10;i++)
	{
		...
		v2->cnt=v.cnt;
		v2->t=malloc(v.cnt*sizeof(task_t));
